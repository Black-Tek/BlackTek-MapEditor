...
void ItemDatabase::setItemAttributes(ItemType* item, const toml::table* attrs)
{
    ...
    if (auto type = attrs->get("type"))
    {
        if (auto typeValue = type->value<std::string>())
        {
            ...
            if (val == "depot")
                ...
                item->type = ITEM_TYPE_MAILBOX;
            else if (val == "trashholder")
                item->type = ITEM_TYPE_TRASHHOLDER;
            else if (val == "container")
                item->type = ITEM_TYPE_CONTAINER;
            else if (val == "door")
                item->type = ITEM_TYPE_DOOR;
            else if (val == "magicfield")
            {
                item->group = ITEM_GROUP_MAGICFIELD;
                item->type = ITEM_TYPE_MAGICFIELD;
            }
    ...
    {
        if (auto desc = description->value<std::string>())
            item->description = *desc;
    }
    if (auto weight = attrs->get("weight"))
    {
        if (auto w = weight->value<int>())
            item->weight = *w / 100.f;
    }
    if (auto armor = attrs->get("armor"))
    {
        if (auto a = armor->value<int>())
            item->armor = *a;
    }
    if (auto defense = attrs->get("defense"))
    {
        if (auto d = defense->value<int>())
            item->defense = *d;
    }
    if (auto rotateTo = attrs->get("rotateTo"))
    {
        if (auto r = rotateTo->value<uint16_t>())
            item->rotateTo = *r;
    }
    if (auto containerSize = attrs->get("containerSize"))
    {
        if (auto cs = containerSize->value<uint16_t>())
            item->volume = *cs;
    }
    if (auto readable = attrs->get("readable"))
    {
        if (auto r = readable->value<bool>())
            item->canReadText = *r;
    }
    if (auto writeable = attrs->get("writeable"))
    {
        if (auto w = writeable->value<bool>())
            item->canWriteText = item->canReadText = *w;
    }
    ...
    {
        if (auto adr = allowDistRead->value<bool>())
            item->allowDistRead = *adr;
    }
    ...
    {
        if (auto bp = blockprojectile->value<bool>())
            item->blockMissiles = *bp;
    }
}
